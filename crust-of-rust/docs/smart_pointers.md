...
## NonNull
A wrapper of `*const T` or `*mut T` but can be sure it is not null. 

## Cow
`Cow` provides flexibility of making a variable either borrowed or owned, using `to_mut()`, it will return a mutable reference to the value, if it is already owned, do nothing, but if borrowed, it will clone the value to be owned. If a borrowed value does not mutate, it is still under borrowed reference, no allocation and clone will happen.
